...
from api.interfaces.storage_service import StorageError, StorageService
...
class SupabaseStorage(StorageService):
    ...
    async def upload_file(
        ...
        **kwargs,
        ...
        try:
            log.info(
                f"Attempting to upload file to bucket: {bucket_name}, path: {file_path}"
            )
            ...
            result = await run_in_threadpool(
                bucket.upload, path=file_path, file=file, file_options=file_options
            )
    ...
    async def download_file(self, bucket_name: str, file_path: str) -> bytes:
        ...
        try:
            result = await run_in_threadpool(
                self.client.storage.from_(bucket_name).download, file_path
            )
    ...
    async def list_files(
        ...
        try:
            bucket = self.client.storage.from_(bucket_name)
            result = (
                await run_in_threadpool(bucket.list, path=path)
                if path
                else await run_in_threadpool(bucket.list)
            )
            return cast(List[Dict[str, Any]], result)
        except StorageException as e:
            raise StorageError(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
