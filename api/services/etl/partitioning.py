...
class PartitionAutoConfig(BaseModel):
    ...
    content_type: Optional[str] = Field(
        default=None, description="File content MIME type"
    )
    file: Optional[IO[bytes]] = Field(
        default=None, description="File-like object in 'rb' mode"
    )
    file_filename: Optional[str] = Field(
        default=None, description="Filename for file object (deprecated)"
    )
    url: Optional[str] = Field(default=None, description="URL of document to parse")
    include_page_breaks: bool = Field(
        default=False, description="Include page break elements"
    )
    strategy: str = Field(
        default=PartitionStrategy.FAST,
    ...
    paragraph_grouper: Optional[Callable[[str], str]] | Literal[False] = Field(
        default=None, description="Function to group paragraphs or False to disable"
    )
    headers: Dict[str, str] = Field(
        default_factory=dict, description="HTTP headers for URL requests"
    )
    ...
    ocr_languages: Optional[str] = Field(
        default=None, description="OCR languages (deprecated)"
    )
    languages: Optional[List[str]] = Field(
        default=None, description="Languages for text detection"
    )
    detect_language_per_element: bool = Field(
        default=False, description="Detect language per element"
    )
    pdf_infer_table_structure: bool = Field(
        default=False, description="Extract table structure (deprecated)"
    )
    extract_images_in_pdf: bool = Field(
        default=False, description="Extract images from PDF (deprecated)"
    )
    extract_image_block_types: Optional[List[str]] = Field(
        default=None, description="Element types to extract as images"
    )
    extract_image_block_output_dir: Optional[str] = Field(
        default=None, description="Directory to save extracted images"
    )
    extract_image_block_to_payload: bool = Field(
        default=False, description="Store images as base64 in metadata"
    )
    xml_keep_tags: bool = Field(default=False, description="Retain XML tags in output")
    data_source_metadata: Optional[DataSourceMetadata] = Field(
        default=None, description="Additional metadata about data source"
    )
    metadata_filename: Optional[str] = Field(
        default=None, description="Custom filename for metadata"
    )
    request_timeout: Optional[int] = Field(
        default=None, description="Timeout for HTTP requests"
    )
    hi_res_model_name: Optional[str] = Field(
        default=None, description="Layout detection model for hi_res strategy"
    )
    model_name: Optional[str] = Field(
        default=None, description="Layout detection model name (deprecated)"
    )
    date_from_file_object: bool = Field(
        default=False, description="Infer last_modified from file bytes"
    )
    starting_page_number: int = Field(
        default=1, description="Starting page number for document"
    )
