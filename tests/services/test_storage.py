...
async def test_storage_flow(storage_service: SupabaseStorage):
    ...
    await storage_service.upload_file(
        bucket_name, "test.txt", test_content, content_type="text/plain"
    )
    ...
    signed_url = await storage_service.create_signed_url(
        bucket_name, "test.txt", expires_in=60
    )
    assert "signedURL" in signed_url

    # Test move operation
    await storage_service.move_file(bucket_name, "test.txt", "moved-test.txt")
